        ...
        if contain_data:
            ...
            if contain_file:
                ...
                if contain_column:
                    ...
                    # Try to implement code here so multiple columns can be selected
                    position = 0
                    while position < len(file_list_input):
                        file_in_use = pd.read_csv('%s%s.csv' % (ROOT_DIR, file_list_input[position]))
                        ...
                        position += 1 ## This line of code if in its finalized version, should not belong here     
