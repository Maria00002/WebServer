...
class requestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        global filename, line_num, header, col_names, time, start_time, end_time, min_timestamp, max_timestamp,int_start_time,int_end_time
        ...
        for filename in dataverse_files.keys():
            for col_name in col_names:
                if self.path.endswith('/AMPds.%s.%s@%d~%d' % (filename,col_name, int(start_time[0]), int(end_time[0]))):
                    self.send_response(200)
                    self.send_header('content-type','text/html')
                    self.end_headers()
                    int_start_time = int(start_time[0])
                    int_end_time = int(end_time[0])
                    output = ''
                    output += '<html><body>'			
                    output += '<h1>starttime</h1>'
                    output += 'Printing start time %d  <br>' % int_start_time
                    output += 'Printing end time %d  <br>' % int_end_time
    ...
    def do_POST(self):
        for filename in dataverse_files.keys():
            for col_name in col_names:
                if self.path.endswith('/AMPds.%s.%s' % (filename,col_name)):
                    ...
                    if ctype == 'multipart/form-data':
                        ...
                        self.send_header('content-type','text/html')
                        self.send_header('Location','/AMPds.%s.%s@%s~%s' % (filename,col_name,start_time[0], end_time[0]))
                        self.end_headers()
