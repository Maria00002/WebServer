...
class requestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        global self_path_list, total_count, stats
        ...
        for filename in dataverse_files.keys():
            ...
            for col_name in col_names:
                if self.path.endswith('/%s/%s' % (filename,col_name)):
                    ...
                    # total read
                    total_count = timestamp_reading['TS'].count()
                    output += '<p>The number of entries for time series data are %d </p>' %(total_count)
                    # standard deviation
                    stats = timestamp_reading.describe()
                    output += '<pre>'
                    output += '<p>The statistical information are %s </p>' %(stats)
                    output += '</pre>'
                    ####
